...
public class PauseMenuTwoPlayer : MonoBehaviour {
	public bool paused;
	...
	void OnGUI(){
		...
		if (!paused) {
			if (GUI.Button (new Rect (Screen.width - Screen.width / 7, Screen.height / 2 - Screen.height / 20, Screen.width / 7, Screen.height / 10), pauseImage, pauseBtnStyle)) {
				PauseGame ();
			}
		} else {
