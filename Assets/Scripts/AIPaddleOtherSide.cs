...
public class AIPaddleOtherSide : MonoBehaviour {
		...
		if (pos == "OtherSide" || pos == null)
			...
			if (slope == 0)
				...
				if (thisTransform.position.x < x) {
					...
					thisTransform.position += Vector3.left * 0.5f;
				}
			}
		} else if (pos == "PuckBehind") {
			thisTransform.position = Vector3.MoveTowards (thisTransform.position, new Vector3 (targTransform.position.x / 2, targTransform.position.y, targTransform.position.z - (targRadius * 8) - 2.0f), step);
		}
	...
	void boundary(){
		...
		if (thisTransform.position.x + thisRadius > 23)
			thisTransform.position = new Vector3 (23.0f - thisRadius, thisTransform.position.y, thisTransform.position.z);
		if (thisTransform.position.z - thisRadius > -5)
	...
	string getPuckAndPaddlePosition(){
		...
		//	targTransform = GameObject.FindGameObjectWithTag ("Puck").transform;
		//}
		if (targTransform.position.z + targRadius > -5)
			return "OtherSide";
